...
#define HANDLE_INPUT_BUFFER                                                                \
  if (LL_LPUART_IsActiveFlag_RXNE(ACTIVE_USART) && LL_LPUART_IsEnabledIT_RXNE(ACTIVE_USART))        \
  {                                                                                        \
    rxBuffer[nextSerialRxIn] =  LL_LPUART_ReceiveData8(ACTIVE_USART);                       \
...
#define HANDLE_OUTPUT_BUFFER                                                               \
  ...
  {                                                                                        \
    if (nextSerialTxOut != nextSerialTxIn) {                                               \
      LL_LPUART_TransmitData8(ACTIVE_USART, txBuffer[nextSerialTxOut]);                     \
