...
//GPS Values
DWfloat Latitude = {"%2.4f", "INVL", 0, 0, false, 0};
DWfloat Longitude = {"%3.3f", "INVL", 0, 0, false, 0};
...
#pragma diag_warning= Pa149
  ...
  case SHOW_COUNT_N_BUTTONS:
    ...
    gpsIcon.yPos =1;
    break;
   case SHOW_GPS_COORDS:
    SSD1306_Clear();
    SSD1306_GotoXY (0,0);
    SSD1306_Puts ("Current GPS", &Font_11x18, SSD1306_COLOR_WHITE);
    SSD1306_GotoXY (0, 20);
    SSD1306_Puts ("LAT: ", &Font_11x18, SSD1306_COLOR_WHITE);           //Set Latitude position on screen
    Latitude.xPos = 44;
    Latitude.yPos = 20;
    SSD1306_GotoXY (0, 41);
    SSD1306_Puts ("LON:", &Font_11x18, SSD1306_COLOR_WHITE);            //Set Longitude position on screen
    Longitude.xPos = 44;
    Longitude.yPos = 41;
    break;
  }
...
void UpdateScreenValues(void)
{
  ...
  case SHOW_GPS_COORDS:
    ...
    SSD1306_GotoXY(Latitude.xPos, Latitude.yPos);
    if (Latitude.valid) {
      sprintf(displayString, Latitude.format, Latitude.data);
      SSD1306_Puts(displayString, &Font_11x18, SSD1306_COLOR_WHITE);
    }
    else
      SSD1306_Puts(Latitude.invalidMsg, &Font_11x18, SSD1306_COLOR_WHITE);

    SSD1306_GotoXY(Longitude.xPos, Longitude.yPos);
    if (Longitude.valid) {
      sprintf(displayString, Longitude.format, Longitude.data);
      SSD1306_Puts(displayString, &Font_11x18, SSD1306_COLOR_WHITE);
    }
    else
      SSD1306_Puts(Longitude.invalidMsg, &Font_11x18, SSD1306_COLOR_WHITE);
    break;
  }
  SSD1306_UpdateScreen(); //display
}
