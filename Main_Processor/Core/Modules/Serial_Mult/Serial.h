...
//Declared a new buffer type enumeration for handling packets
typedef enum _comm_buff_id_ {
  GPS_COM,
  RS485_COM,
  PC_COM,
  FO_COM
...
typedef struct _comm_buffer_t {
  ...
  uint8_t packetBuffer[16];     //Packet Buffer
  comm_buff_id buff_type;       //Buffer Type
  uint8_t inPacket;
  uint8_t nextPacketChar;
  uint8_t PacketRdy;
  USART_TypeDef *portInstance;
} comm_input_buffer_t;

typedef struct _comm_buffer_out_t {
  uint8_t nextBufferIn;
  uint8_t nextBufferOut;
  uint8_t * buffer;
  uint8_t bufferLength;
  comm_buff_id buff_type;
  USART_TypeDef *portInstance;
